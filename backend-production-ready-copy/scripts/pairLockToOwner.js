/**
 * سكربت ربط القفل الذكي بحساب المالك
...
async function pair() {
  try {
    ...
    const allowedDevice = await AllowedDevice.findOne({ serialNumber: LOCK_SERIAL });
    if (!allowedDevice) {
      console.error(`AllowedDevice not found: ${LOCK_SERIAL}`);
      console.error('Run seedLockDevice.js first.');
      process.exit(1);
    }

    let device = await Device.findOne({ serialNumber: LOCK_SERIAL });

    if (device) {
      console.log(`Device ${LOCK_SERIAL} exists, updating owner...`);
      device.owner = owner._id;
      device.pairedAt = new Date();
      device.name = device.name || 'Smart Lock';
      ...
      console.log(`Creating device ${LOCK_SERIAL}...`);
    ...
    console.log('\n========================================');
    console.log('   Smart Lock Paired to Owner!');
    console.log('========================================');
