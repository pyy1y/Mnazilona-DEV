/**
 * ═══════════════════════════════════════════════════════════════
 *  Mnazilona IoT - ESP32-C6 Firmware v1.1.0 (Wi-Fi AP variant)
 *  SoftAP Provisioning (no PoP) + DeviceSecret
 * ═══════════════════════════════════════════════════════════════
 *
 *  Identical to GarageRelayFirmware.ino EXCEPT the provisioning
 *  layer: BLE removed, replaced with Wi-Fi SoftAP + HTTP.
 *
 ...
 *  BLE firmware so the backend, app, and admin dashboard see no
...
// SSID is "MNZ_<serial>" — same naming convention as the BLE
// firmware so the app can identify Mnazilona devices uniformly.
#define AP_SSID_PREFIX     "MNZ_"
// AP password. Min 8 chars for WPA2; leave empty string for open AP.
// Leaving open is simpler for the user (no password to type) but some
// Android versions show a "no internet" warning that confuses users.
// Defaulting to a known fixed WPA2 password — the app can show it on
// screen so the user can copy/paste, or auto-fill it via platform APIs.
...
// want the HTTP response to ship cleanly first. Same handoff pattern
// the BLE firmware uses.
...
// the HTTP handler (heavy/blocking work in handleClient stalls the
// server).
...
enum DeviceState {
  STATE_BOOT,
  STATE_AP_ACTIVE,            // SoftAP up, awaiting Wi-Fi credentials
  STATE_AP_CONNECTING_STA,    // Credentials received, attempting STA
...
WebServer        provisionServer(AP_HTTP_PORT);  // AP-mode HTTP provisioning server
WebServer        localServer(8080);              // Local API server (post-provisioning)
...
bool connectWiFi(const String& ssid, const String& psw, uint32_t timeoutMs = 12000) {
  ...
  // Switch to STA-only — provisioning AP (if any) is torn down by the
  // caller. We do NOT use AP_STA here because the radio sharing causes
  // STA channel hops that destabilize early connection on ESP32-C6.
  WiFi.mode(WIFI_STA);
...
bool connectMQTT() {
  ...
  if (mqttClient.connect(cid.c_str(), mqttUser.c_str(), mqttPass.c_str(),
    ...
    if (otaPendingVerify) confirmOtaFirmware();
...
//    Same as BLE firmware — runs on
//    port 8080 once the device is online.
// ══════════════════════════════════════
...
// ══════════════════════════════════════
//
// Endpoints (all on http://192.168.4.1:80 while AP is up):
//   GET  /info     → device info JSON (incl. deviceSecret)
//   GET  /scan     → scan for nearby Wi-Fi networks (synchronous, ~3-5s)
//   POST /config   → save Wi-Fi creds, accept STA connection attempt
//   GET  /status   → current provisioning state
//   GET  /health   → simple ping for the app to confirm AP reachability
//
// Liberal CORS — the LAN AP is a one-shot trust boundary, the user
// is physically connected to the device's own SSID. Same posture as
// the BLE provisioning channel.
...
static void apHandleInfo() {
  ...
  doc["deviceSecret"] = deviceSecret;       // identical role to BLE PoP-verify response
...
static void apHandleScan() {
  ...
  // Switch to AP_STA so the STA radio can perform the scan while the
  // AP keeps serving HTTP. Returns to AP-only after the scan to keep
  // the AP channel stable until /config arrives.
  ...
  // Drop STA half — AP-only keeps the channel pinned for the next
  // handler (/config) which the app is about to call.
  WiFi.mode(WIFI_AP);
...
static void apHandleConfig() {
  ...
  // Defer the actual STA connection. We respond immediately so the
  // app reliably reads the response while still on the AP, then the
  // main loop (outside the HTTP handler) handles AP teardown + STA
  // connect + server inquiry + MQTT. Same handoff pattern the BLE
  // firmware uses.
...
static void apHandleNotFound() {
  apSendCors();
  // Captive-portal helpers: many phones probe well-known URLs to
  // detect "no internet". Always returning 200 keeps Android/iOS
  // from kicking the user off the AP — without it, Android may
  // silently switch back to mobile data mid-provisioning.
...
void startAP() {
  ...
  // Pure AP mode for advertising. /scan briefly switches to AP_STA
  // and back. Cleanest channel behavior for ESP32-C6.
  WiFi.mode(WIFI_AP);
  bool ok;
  ...
  // Register routes once. WebServer::on() appends to an internal list,
  // so calling it on every startAP() (e.g. after a failed STA retry)
  // would create duplicate handlers.
...
void stopAP() {
  ...
  // Caller (loop / connectWiFi) sets the next mode (typically WIFI_STA).
  Serial.println("[AP] Provisioning stopped");
  addLog("AP provisioning stopped", "info");
}

// Legacy aliases (none of the rest of the code uses them, but kept for
// parity with the BLE firmware's startBLE/stopBLE)
...
// ══════════════════════════════════════
// Runs in the main loop (NOT in an HTTP handler). When /config sets
// apConfigPending we wait briefly for the response to ship, then bring
// the AP down and try to join the user's home Wi-Fi.

void handleApConfigPending() {
  if (!apConfigPending) return;

  // Give the HTTP response time to flush to the app, and let the app
  // render its "device is connecting…" UI before we kill the AP.
  ...
  // Stop AP cleanly. From this point the app's HTTP socket on
  // 192.168.4.1 is gone — the app is expected to disconnect from the
  // AP and start polling the backend.
  stopAP();
  delay(300);
  ...
  if (!connected) {
    ...
    // Wipe the bad credentials so the next /config attempt is clean,
    // but keep the device identity. Restart AP for a fresh provisioning
    // session.
    prefs.begin("config", false);
    prefs.remove("ssid");
    prefs.remove("psw");
  ...
  // STA up → mark for the main loop to do server inquiry + MQTT.
  // Same flag the BLE firmware uses, so the rest of the state machine
  // is identical.
...
void handleStateMachine() {
  // Wi-Fi → server inquiry → MQTT handoff (same flag the BLE firmware uses).
  ...
  switch (currentState) {
    ...
    case STATE_AP_ACTIVE:
      // Wi-Fi AP provisioning: pump the HTTP server and watch for the
      // /config trigger that fires the deferred STA attempt.
      ...
      if (apProvisioning && millis() - apLastActivity > AP_TIMEOUT_MS) {
        ...
        if (storedSSID.length() > 0) {
          ...
          apLastActivity = millis();  // keep advertising indefinitely if no saved Wi-Fi
        }
      }
      break;
    ...
    if (connectWiFi(storedSSID, storedPassword)) {
      ...
      if (mqttHost.length() == 0) {
        ...
        if (inqResult == INQUIRY_OWNED) {
          ...
          // No backend yet — fall back to AP for re-provisioning
          startAP();
        }
      } else {
        currentState = STATE_ONLINE;
